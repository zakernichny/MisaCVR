...
namespace PlayerList.Components
{
    internal class PlayerList : MonoBehaviour
    {
        ...
        void OnPlayerJoin(CVRPlayerEntity player)
        {
            ...
            //var button = playerEntry.AddComponent<Button>();
            var button = playerEntry.AddComponent<Button>();

            button.onClick.AddListener(new UnityEngine.Events.UnityAction(new Action(() => {
                _logger.Msg($"[TP] Player: {playerData.Name}[{playerData.Id}][{player.DarkRift2Player.Position}]");
